...
import { createProductWithVariants } from "@/app/admin/(panel)/products/actions";
...
import type { ProductStatus, VariantRowState } from "@/types/product";
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await createProductWithVariants({
        ...
        status,
        imageUrls,
        variants: parsedVariants.map(({ key: _key, ...variant }) => variant),
      });
